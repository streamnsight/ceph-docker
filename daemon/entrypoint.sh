...
function osd_directory {
  ...
  for OSD_ID in $(ls /var/lib/ceph/osd |  awk 'BEGIN { FS = "-" } ; { print $2 }'); do
    ...
    if [ ! -e /var/lib/ceph/osd/${CLUSTER}-${OSD_ID}/keyring ]; then
      echo "Create OSD key and file structure for OSD with ID ${OSD_ID}"
      ceph-osd ${CEPH_OPTS} --id ${OSD_ID} --mkfs --mkkey --mkjournal --osd-journal ${OSD_J} --setuser ceph --setgroup ceph -d
